...
class MiniMax_subpage extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
                'minimax_subpage', // Base ID
                __('Subpages', 'minimax'), // Name
                array('description' => __('List Sub-Pages', 'minimax'),) // Args
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        if (empty($pageid))
            $pageid = get_the_ID();
        ?>
        <style> .widget_minimax_subpage ul li{list-style:none;}.page-list-<?php echo $xid; ?> li{ list-style:none;padding:0px;margin:0px;padding-left: 20px; background: url('<?php echo base_theme_url . '/modules/subpages/icons/' . $sicon; ?>') left center no-repeat; line-height: 22px; }</style>
        <ul class="page-list-<?php echo $xid; ?>">
        <?php wp_list_pages("title_li=&child_of=$pageid&show_date=modified"); ?>
        </ul>
